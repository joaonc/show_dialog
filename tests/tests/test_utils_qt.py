...
class TestListResources:
    def test_resource_path(self, testing_resources):
        ...
        assert files == [':/stylesheets/style_02.css']

    def test_absolute_path(self):
        base_path = TEST_ASSETS_DIR / 'stylesheets'
        files = list_resources(base_path)
        assert files == [f'{base_path}/style_02.css']
...
class TestReadFile:
    def test_read_file(self, testing_resources):
        file_content = read_file(':/stylesheets/style_02.css')
        assert 'Test stylesheet' in file_content

    def test_absolute_path(self):
        file_content = read_file(TEST_ASSETS_DIR / 'stylesheets/style_02.css')
        assert 'Test stylesheet' in file_content
